-- =====================================================================
-- 兜底策略模板（rule_type=4 = FALLBACK）
-- 由 FallbackRuleEngine 按画像（goalType + pathType）匹配，FallbackService 填充占位符。
-- 占位符：{major}（专业）、{graduationYear}（毕业年份）。
-- salaryExpectation 不写，由 PlanPersistenceService.resolveSalary 走薪资规则引擎兜底。
-- 已有库请手动执行本文件；全新安装可合并进 sql.sql。
-- =====================================================================
